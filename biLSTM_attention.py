...
class AttentionLayer(nn.Module):
    ...
    def forward(self, outputs):
        attention_scores = self.attention_weights(outputs).squeeze(-1)
        attention_weights = F.softmax(attention_scores, dim = 1)

        weighted_output = torch.bmm(attention_weights.unsqueeze(1), outputs).squeeze(1)
...
class biLSTM_Simple(nn.Module):
    def __init__(self, embedding_matrix_1, embedding_dimension, vocab_size, hidden_size, output_size, num_layers = 1, dropout = 0):
        ...
        self.embedding.weight.requires_grad = True # UnFreezings the embeddings
        ...
        self.biLSTM = nn.LSTM(embedding_dimension, hidden_size, num_layers, bidirectional=True, batch_first=True, dropout=dropout)
