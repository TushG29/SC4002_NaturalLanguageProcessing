...
# Function to build and train model for each configuration of depth and width
def train_with_varying_depth_width():
    depths = [1, 2, 3]
    widths = [64, 128, 256]
    ...
    for depth in depths:
        for width in widths:
            ...
            # Initialize the model with specific depth and width
            model = RNN_Simple(embedding_matrix_1=embedding_matrix, embedding_dimension=embedding_dim,
                               vocab_size=vocab_size, hidden_size=width, output_size=output_dim, num_layers=depth)
            ...
            for epoch in range(epochs):
                ...
                print(f"Depth {depth}, Width {width}, Epoch {epoch+1}/{epochs} - Training loss: {train_loss:.4f}, Validation loss: {valid_loss:.4f}, Test loss: {test_loss:.4f}")
                print(f"Training Accuracy: {train_correct*100:.1f}%, Validation Accuracy: {valid_correct*100:.1f}%, Test Accuracy: {test_correct*100:.1f}%")
            ...
            final_test_loss, final_test_correct = test_loop(model, test_loader, loss_fn)
            print(f"Final Test loss for depth={depth}, width={width}: {final_test_loss:.4f}")
            print(f"Final Test Accuracy for depth={depth}, width={width}: {final_test_correct*100:.1f}%")
            print("--------------------------------------------------------")

# Run the function to train models with varying depths and widths
train_with_varying_depth_width()
